    ...
    void update(float p_decision_w, float hot_water_temp, time_t now) {
        ...
        if (!on) {
            ...
            if (p_decision_w < TURN_ON_THRESHOLD_W) {
                if (below_on_since == 0) below_on_since = now;
            ...
            if (p_decision_w > TURN_OFF_THRESHOLD_W) {
                if (above_off_since == 0) above_off_since = now;
...
int main() {
    ...
    while (g_running) {
        ...
        int rc2 = (rc1 == -1) ? -1 :
                  modbus_read_input_registers(ctx, TOTAL_P_START, TOTAL_P_COUNT, total_p_regs);
        ...
        if (rc1 == -1 || rc2 == -1) {
            ...
            if (wait > MAX_BACKOFF_SEC) wait = MAX_BACKOFF_SEC;
            sleep(wait);
            continue;
        }
        ...
        heater.update(p_total, g_hot_water_temp.load(), time(nullptr));
